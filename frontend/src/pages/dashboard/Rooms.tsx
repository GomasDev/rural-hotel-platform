  ...
  useEffect(() => {
    ...
    if (selectedHotel === 'all') {
      ...
      fetch(`${API}/rooms/hotel/${selectedHotel}`)
        .then(res => res.json())
        .then(data => setRooms(Array.isArray(data) ? data.map((r: Room) => ({ ...r, hotelId: selectedHotel })) : []))
        .catch(() => setError('Error al cargar habitaciones'))
        .finally(() => setLoadingRooms(false));
    }
  }, [selectedHotel]);
  ...
  return (
    <>
        ...
        {loadingRooms ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {rooms.map(room => {
              ...
              return (
                <div key={room.id} className="group bg-white rounded-2xl overflow-hidden hover:shadow-lg transition-shadow flex flex-col">
                  {/* Imagen */}
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    {selectedHotel === 'all' && hotelName && (
                      <p className="text-xs text-green-600 font-semibold mb-1">🏨 {hotelName}</p>
                    )}
      ...
      {bookingModal.open && bookingModal.room && (
        <BookingModal
          room={{
            id:            bookingModal.room.id,
            name:          bookingModal.room.name,
            pricePerNight: String(bookingModal.room.pricePerNight),
            capacity:      bookingModal.room.capacity,
          }}
          hotelName={hotels.find(h => h.id === bookingModal.room?.hotelId)?.name ?? ''}
